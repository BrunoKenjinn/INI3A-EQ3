...
export default function TelaNotificacoes({ navigation }) {
  ...
  const handleMarcarComoLida = async (id: number) => {
    if (filtroStatus === "lidas") return;

    try {
      const token = await AsyncStorage.getItem("auth_token");

      await axios.patch(
        `${url}/api/notificacoes/${id}/lida`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const agruparNotificacoes = () => {
    ...
    const grupos: { [key: string]: Notificacao[] } = {
      Hoje: [],
      Ontem: [],
      "Esta Semana": [],
      "Este Mês": [],
    ...
    notificacoes.forEach((notificacao) => {
      ...
      if (dataNotificacao.getTime() === hoje.getTime())
        grupos["Hoje"].push(notificacao);
      else if (dataNotificacao.getTime() === ontem.getTime())
        grupos["Ontem"].push(notificacao);
      else if (dataNotificacao >= inicioSemana)
        grupos["Esta Semana"].push(notificacao);
      else if (
        dataNotificacao.getMonth() === hoje.getMonth() &&
        dataNotificacao.getFullYear() === hoje.getFullYear()
      )
        grupos["Este Mês"].push(notificacao);
        ...
        <SectionList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>
              {`Nenhuma notificação ${
                filtroStatus === "lidas" ? "lida" : "não lida"
              } encontrada.`}
            </Text>
          }
        />
